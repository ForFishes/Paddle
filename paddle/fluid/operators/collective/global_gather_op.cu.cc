...
class GlobalGatherOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto fwd_count = lec_cumsum[cpu_local_count.size() - 1];
    ...
    //   expert_ptr[i] = expert_ptr[i - 1] + cpu_local_count[i - 1];
    // }
    auto send_ptr = 0;
      ...
      for (auto j = 0; j < nranks; ++j) {
        ...
        if (cpu_global_count[idx]) {
          PADDLE_ENFORCE_GPU_SUCCESS(platform::dynload::ncclSend(
              send_buf + send_ptr * in_feat, cpu_global_count[idx] * in_feat,
              dtype, j, comm->comm(), stream));
          send_ptr += cpu_global_count[idx];
        }
        if (cpu_local_count[idx]) {
          int64_t offset = idx > 0 ? lec_cumsum[idx - 1] : 0;
          PADDLE_ENFORCE_GPU_SUCCESS(platform::dynload::ncclRecv(
              recv_buf + offset * in_feat, cpu_local_count[idx] * in_feat,
              dtype, j, comm->comm(), stream));
        }
      }
