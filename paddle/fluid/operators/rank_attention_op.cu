...
class RankAttentionCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    //        ctx.cuda_device_context().stream(), X->data<T>(), ins_num,
    //        x_fea_dim,
    //        input_help_data, ins_num, block_matrix_row,
    //        rank_offset->data<int>(),
    //        rank_offset_dims[0], rank_offset_dims[1], ins_rank_data,
    //        max_rank);
    //
    get_rank_attention_insrank(ctx.cuda_device_context().stream(),
                               rank_offset->data<int>(), rank_offset_dims[0],
                               rank_offset_dims[1], ins_rank_data, ins_num);
    ...
    //                 input_help_data, param_help_data, beta, out_data,
    //                 ins_num,
    //                 strideA, strideB);
    // FIXME it wastes time to calculate and need to sort.

    std::vector<T> v_ins_rank;
    framework::TensorToVector(*ins_rank, dev_ctx, &v_ins_rank);

    std::cerr << "v_ins_rank" << std::endl;
    for (auto ele : v_ins_rank) {
      std::cerr << ele << " ";
    }
    std::cerr << std::endl;

    std::vector<int> input_pos(ins_num, -1);
    std::vector<int> input_offset(ins_num, -1);

    for (int index = 0; index < ins_num; ++index) {
      int rank = static_cast<int>(v_ins_rank[index]);
      if (rank == -1) continue;
      if (rank == 1) {
        input_pos[index] = index;
      } else if (rank > 1) {
        assert(index - 1 >= 0);
        input_pos[index] = input_pos[index - 1];
      }
    }
    ...
    while (true) {
      while (input_pos[fast] == input_pos[low]) ++fast;
      int offset = fast - low;
      for (; low < fast; ++low) {
        input_offset[low] = offset;
      }
...
//
//    auto *drank_para =
//    ctx.Output<Tensor>(framework::GradVarName("RankParam"));
//
//    // get dim
//    auto x_dims = X->dims();
//    auto ins_num = x_dims[0];
